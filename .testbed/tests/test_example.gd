...
const EXPECTED_PLUGIN_DESCRIPTION := "Template for AeroBeat UI kit repos. Shared visual components built on aerobeat-ui-core for current v1 UI work."
...
func test_readme_keeps_v1_ui_kit_template_truth() -> void:
	var readme_text := _read_repo_file(README_PATH)
	assert_true(readme_text.contains("official template for creating **UI kit** repositories"), "README should state that this repo is a UI kit template")
	...
	assert_true(readme_text.contains("shared visual components and presentation patterns belong here"), "README should preserve the UI-kit lane boundary")
...
func test_plugin_cfg_description_stays_template_specific() -> void:
	...
	assert_eq(config.get_value("plugin", "name", ""), "AeroBeat UI Kit Template", "plugin.cfg name should stay stable")
	assert_eq(
		config.get_value("plugin", "description", ""),
		EXPECTED_PLUGIN_DESCRIPTION,
		"plugin.cfg description should remain aligned with the template's narrow v1 UI-kit contract"
	)
